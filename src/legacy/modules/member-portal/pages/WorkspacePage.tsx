/*
    ERP-CLUB - ERP pour Club de vol à voile
    - Logiciel libre de gestion d'un club de vol à voile
    - member-portal: Workspace page — wraps the shared MemberWorkspaceShell in portal mode
    Copyright (C) 2026  SAFORCADA Patrick

    This program is free software: you can redistribute it and/or modify
    it under the terms of the GNU Affero General Public License as published
    by the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    This program is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU Affero General Public License for more details.

    You should have received a copy of the GNU Affero General Public License
    along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */
import { Navigate } from 'react-router-dom';

import { MemberWorkspaceShell } from '../../members/components/MemberWorkspaceShell';
import { getPortalProfile } from '../api/client';

export function WorkspacePage() {
  const profile = getPortalProfile<{ uuid: string }>();

  if (!profile) {
    return <Navigate to="/member-portal/login" replace />;
  }

  return <MemberWorkspaceShell mode="portal" memberUuid={profile.uuid} />;
}
